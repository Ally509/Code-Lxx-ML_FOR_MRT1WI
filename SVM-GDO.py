...
def run(seed, X1, y1):
    ...
    #
    # x_train, y_train = GDO(k=5, alpha=1).fit_sample(x_train, y_train)
    # print("the number of x_train:",y_train.shape)



    # Datas Splitting2
    x_train0, x_test, y_train0, y_test = train_test_split(X_rfe, y1, test_size=0.2, random_state=seed)

    x_over, y_over = GDO(k=5, alpha=1).fit_sample(X_rfe, y1)
    ...
    # Classifier
    # clf = ExtraTreesClassifier(random_state=42)
    clf = SVC(random_state=42,gamma='scale',class_weight='balanced')
